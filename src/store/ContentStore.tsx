...
type Ctx = { content: SiteContent; draft: SiteContent; updateDraft: (path: string, value: any) => void; saveChanges: () => void; discardChanges: () => void; hasChanges: boolean; userRole: string | false; isAdmin: boolean; login: (usr: string, pw: string) => { ok: boolean; error?: string }; logout: () => void; editMode: boolean; setEditMode: (b: boolean) => void; exportJSON: () => void; importJSON: (file: File) => Promise<void>; sessionExpiresIn: number; };
...
export function ContentProvider({ children }: { children: ReactNode }) {
  ...
  return <ContentContext.Provider value={{ content: displayContent, draft, updateDraft, saveChanges, discardChanges, hasChanges, userRole, isAdmin: Boolean(userRole), login, logout, editMode, setEditMode, exportJSON, importJSON, sessionExpiresIn }}>{children}</ContentContext.Provider>;
}
